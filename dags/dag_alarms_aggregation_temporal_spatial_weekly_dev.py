# Nome da DAG: dag_alarms_aggregation_temporal_spatial_weekly_dev
# Owner / responsável: CoE
# Descrição do objetivo da DAG: 
# 1. Busca en Postgres Dev los thesholds con granularidad = semana
...
# Frequência de execução (schedule): Cada semana
# Dag Activo?: No
# Autor: CoE
...
def data_load_control_kpi(**kwargs):    
    ...
    result=functions.postgresData("dataAggregationTime","WEEK")
    functions.prepared_data_load(result,var_datetime,"WEEK",0)


# Función para realizar la consulta del dataframe
def query_select_datataframe(**kwargs):
    resut_json=functions.postgresData("dataAggregationTime","WEEK")
    return resut_json
...
def druid_adapter_query(param_df_druid, param_start_date, **kwargs): 
    ...
    sql_queries_json=functions.druid_adapter_query(param_df_druid=param_df_druid,aggregationTime="WEEK",minutos=0,var_datetime=var_datetime)
...
def produce_message_to_topic_kafka(**kwargs):
    ...
    functions.produce_message_to_topic_kafka(response_texts,604800,"PT604800S")
    

#Definición del Dag
dag = DAG(
    'dag_alarms_aggregation_dev_temporal_spatial_weekly',  # Nombre de tu DAG
    default_args=default_args,
    schedule_interval='30 0 * * 1',  # Se ejecuta todos los lunes a las 12:30 am
    catchup=False,
