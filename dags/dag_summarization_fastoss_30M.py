# Nome da DAG: dag_summarization_fastoss_30M
# Owner / responsável: CoE
# Descrição do objetivo da DAG: Ejecuta una tarea index_parallel en Druid para leer datos desde fastoss-pm-enriched-metrics y agregarlos en fastoss-pm-enriched-metrics-temporal-30m cada 30 minutos, procesando métricas con granularidad de 15 minutos en el ambiente de producción (prd)
# 
# Nota: 
...
dag = DAG('dag_summarization_fastoss_30M',
...
with dag:
    ...
    def calculate_summarization(var_execution_date): 
        response = functions.druid_create_task_summarization(var_execution_date,"30M","temporal","fastoss","load")
        
        minute_block = (var_execution_date.minute // 30) * 30
    ...
    def verify_and_retry_druid_task(**context):
        ...
        try:
            ...
            if status == 'FAILED':
                ...
                recovery_response = functions.druid_create_task_summarization(var_execution_date,"30M","temporal","fastoss","load")
    ...
    trigger_hourly_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id='dag_summarization_fastoss_hourly', 
        conf={
            'interval': '1H',
