...
dag = DAG('dag_summarization_snmp_group_15M',
...
with dag:
    ...
    def calculate_summarization(var_execution_date): 
        response = functions.druid_create_task_summarization(var_execution_date, "15M", "spatial", "snmp", "load")
        
        minute_block = (var_execution_date.minute // 15) * 15
        is_last_interval = (minute_block == 0)
        
        return {
            'task_druid_id': response['queryId'],
    ...
    def verify_and_retry_druid_task(**context):
        ...
        var_execution_date = datetime.strptime(execution_date, "%Y-%m-%d %H:%M:%S") 
        ...
        try:
            ...
            if status == 'FAILED':
                ...
                recovery_response = functions.druid_create_task_summarization(var_execution_date, "15M", "spatial", "snmp", "load")
    ...
    trigger_hourly_dag = TriggerDagRunOperator(
        ...
        trigger_dag_id='dag_summarization_snmp_group_hourly', 
        conf={
            'interval': '1H',
