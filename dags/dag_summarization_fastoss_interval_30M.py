# Nome da DAG: dag_summarization_fastoss_interval_30M
# Owner / responsável: CoE
# Descrição do objetivo da DAG: Reprocesa las agregaciones temporal de 30M para un periodo de tiempo estipulado
#
# Nota: 
...
def generar_tareas_druid(fecha, **kwargs):
    functions.druid_create_task_summarization(fecha,"30M", "temporal", "fastoss", "interval")    
 
# Función para insertar un retraso
def esperar():
    time.sleep(5 * 60)  # Espera 30 minutos
...
with DAG(
    "dag_summarization_fastoss_interval_30M",
    ...
    with TaskGroup("grupo_subtareas") as grupo:
        start_date = Variable.get("summarization_fastoss_date_start")  
        end_date = Variable.get("summarization_fastoss_date_end")      
        intervals=functions.date_creator(start_date, end_date, timedelta(minutes=30)) 
        num_subtareas = len(intervals)

        # Crear las tareas y configurarlas en grupos
