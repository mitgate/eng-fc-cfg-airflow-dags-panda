# Nome da DAG: dag_summarization_snmp_interval_group_hourly
# Owner / responsável: CoE
# Descrição do objetivo da DAG: Reprocesa las agregaciones por grupo de Hora para un periodo de tiempo estipulado
# 
# Nota: 
...
def generar_tareas_druid(fecha, **kwargs):
    functions.druid_create_task_summarization(fecha,"HOURLY","spatial","snmp","interval")    

# Función para insertar un retraso
def esperar():
    time.sleep(3 * 60)  # Espera 
...
with DAG(
    "dag_summarization_snmp_interval_group_hourly",
    ...
    with TaskGroup("grupo_subtareas") as grupo:
        ...
        intervals=functions.date_creator(start_date,end_date,timedelta(hours=1)) 
