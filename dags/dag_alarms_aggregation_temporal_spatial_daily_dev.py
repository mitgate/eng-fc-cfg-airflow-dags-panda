# Nome da DAG: dag_alarms_aggregation_temporal_spatial_daily_dev
# Owner / responsável: CoE
# Descrição do objetivo da DAG: 
# 1. Busca en Postgres Dev los thesholds con granularidad = día
...
# Frequência de execução (schedule): Diario
# Dag Activo?: No
# Autor: CoE
...
def data_load_control_kpi(**kwargs):    
    ...
    result=functions.postgresData("dataAggregationTime","DAY")
    functions.prepared_data_load(result,var_datetime,"DAY",0)


# Función para realizar la consulta del dataframe
def query_select_datataframe(**kwargs):
    resut_json=functions.postgresData("dataAggregationTime","DAY")
    return resut_json
...
def druid_adapter_query(param_df_druid, param_start_date, **kwargs): 
    ...
    sql_queries_json=functions.druid_adapter_query(param_df_druid=param_df_druid,aggregationTime="DAY",minutos=0,var_datetime=var_datetime)
    return sql_queries_json  
     
def execute_druid_adapter_query(**kwargs):
    df_druid = kwargs['param_df']
    start_date=kwargs['start_date']
    #start_date = '2024-06-10 01:01:00.000Z'
...
def produce_message_to_topic_kafka(**kwargs):
    ...
    functions.produce_message_to_topic_kafka(response_texts,86400,"PT86400S")
    

#Definición del Dag
dag = DAG(
    'dag_alarms_aggregation_dev_temporal_spatial_daily',  # Nombre de tu DAG
    default_args=default_args,
    schedule_interval='30 0 * * *',  # Se ejecuta a las 12.30 am de cada día
    catchup=False,
