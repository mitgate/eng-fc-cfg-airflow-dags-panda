# Nome da DAG: dag_alarms_aggregation_temporal_spatial_hourly_dev
# Owner / responsável: CoE
# Descrição do objetivo da DAG: 
# 1. Busca en Postgres Dev los thesholds con granularidad = hora
...
def data_load_control_kpi(**kwargs):    
    ...
    result=functions.postgresData("dataAggregationTime","HOUR")
    functions.prepared_data_load(result,var_datetime,"HOUR",0)


# Función para realizar la consulta del dataframe
def query_select_datataframe(**kwargs):
    resut_json=functions.postgresData("dataAggregationTime","HOUR")    
    return resut_json
...
def druid_adapter_query(param_df_druid, param_start_date, **kwargs): 
    ...
    sql_queries_json=functions.druid_adapter_query(param_df_druid=param_df_druid,aggregationTime="HOUR",minutos=0,var_datetime=var_datetime)
...
def execute_druid_adapter_query(**kwargs):
    ...
    #start_date = '2024-06-09 12:31:00.000Z'
...
def produce_message_to_topic_kafka(**kwargs):
    ...
    functions.produce_message_to_topic_kafka(response_texts,3600,"PT3600S")
    

#Definición del Dag
dag = DAG(
    'dag_alarms_aggregation_dev_temporal_spatial_hourly',  # Nombre de tu DAG
    default_args=default_args,
    schedule_interval='15 * * * *',  # Se ejecuta en el minuto 15 de cada hora.
    catchup=False,
