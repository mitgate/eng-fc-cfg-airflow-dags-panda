...
dag = DAG('dag_busyHour_dev_monthly_v2',
...
with dag:
    ...
    def create_query_groups(param_df, param_execution_date):
        json_file_path = functionsBH.create_query_groups(param_df, param_execution_date, periodicity)
    
        # Leer el JSON para obtener el número de registros
        with open(json_file_path, "r") as f:
            df_dict = json.load(f)
        
        # Devolver la ruta del archivo y el número de registros
        return {
    ...
    @task
    def calculate_busyHour_row(file_path, index):
        with open(file_path, "r") as f:
            df_dict = json.load(f)
        
        # Procesar la fila específica
        fila = df_dict[index]
        functionsBH.calculate_busyhour(fila, index)
    ...
    create_query_groups_task = create_query_groups(
        param_df=get_busyHour_configuration_task, 
        param_execution_date=setting_variables_task
    )

    # with TaskGroup("calculate_busyHour_tasks", prefix_group_id=False) as calculate_busyHour_tasks:
    #     calculate_busyHour_task = calculate_busyHour_row.expand(
    #         #file_path=[create_query_groups_task['file_path']] * create_query_groups_task['total_records'],
    #         index=list(range(create_query_groups_task['total_records']))
    #     )
    ...
    get_busyHour_configuration_task >> create_query_groups_task >> end_task #calculate_busyHour_tasks 
