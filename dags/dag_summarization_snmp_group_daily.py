# Nome da DAG: dag_summarization_snmp_group_daily
# Owner / responsável: CoE
# Descrição do objetivo da DAG: Ejecuta una tarea index_parallel en Druid para leer datos desde snmp-enriched-metrics-group-hourly y agregarlos en snmp-enriched-metrics-group-daily-3, consolidando métricas diarias en el ambiente de producción (prd)
#
# Nota: 
...
dag = DAG('dag_summarization_snmp_group_daily',
...
with dag:
    ...
    def calculate_summarization(var_execution_date): 
        response = functions.druid_create_task_summarization(var_execution_date, "DAILY", "spatial", "snmp", "load")
    ...
    def verify_and_retry_druid_task(**context):
        ...
        try:
            ...
            if status == 'FAILED':
                ...
                recovery_response = functions.druid_create_task_summarization(var_execution_date, "DAILY", "spatial", "snmp", "load")
