# Nome da DAG: dag_alarms_aggregation_spatial_1H
# Owner / responsável: CoE
# Descrição do objetivo da DAG: 
# 1. Busca en Postgres Prd los thesholds con granularidad = 60 minutos y agregación temporal = None 
...
def data_load_control_kpi(**kwargs):    
    ...
    result=functions.postgresData("dataAggregationTimeGranularity","PT1H")
    functions.prepared_data_load(result,var_datetime,"PT1H",60)


# Función para realizar la consulta del dataframe
def query_select_datataframe(**kwargs):
    resut_json=functions.postgresData("dataAggregationTimeGranularity","PT1H")
    return resut_json
...
def druid_adapter_query(param_df_druid, param_start_date, **kwargs): 
    ...
    sql_queries_json=functions.druid_adapter_query(param_df_druid=param_df_druid,aggregationTime="",minutos=60,var_datetime=var_datetime)
...
#Definición del Dag
dag = DAG(
    'dag_alarms_aggregation_spatial_1H',  # Nombre de tu DAG
