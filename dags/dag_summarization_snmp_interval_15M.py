# Nome da DAG: dag_summarization_snmp_interval_15M
# Owner / responsável: CoE
# Descrição do objetivo da DAG: Reprocesa las agregaciones temporal de 15M para un periodo de tiempo estipulado
#
# Nota: 
...
def generar_tareas_druid(fecha, **kwargs):
    functions.druid_create_task_summarization(fecha, "15M", "temporal", "snmp", "interval")    

# Función para insertar un retraso
def esperar():
    time.sleep(9 * 60)  # Espera 30 minutos
...
with DAG(
    "dag_summarization_snmp_interval_15M",
    ...
    with TaskGroup("grupo_subtareas") as grupo:
        start_date = Variable.get("summarization_snmp_date_start")  
        end_date = Variable.get("summarization_snmp_date_end")      
        intervals=functions.date_creator(start_date, end_date, timedelta(minutes=15)) 
