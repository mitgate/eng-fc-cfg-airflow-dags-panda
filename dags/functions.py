...
connection_postgres = BaseHook.get_connection('postgres_prd')
engine = create_engine(f"postgresql://{connection_postgres.login}:{connection_postgres.password}@{connection_postgres.host}:{connection_postgres.port}/{connection_postgres.schema}")
...
def postgresData(aggretation, granularity):
    ...
    query = f"""               
        SELECT node_id, node_name, "dataAggregationTime", "dataAggregationTimeGranularity", 
            ...
            "kpi_quarterly", "kpi_semesterly", execution_date, load_status,"timeZone"
        FROM panda.threshold
...
def get_cadena_query_adapter(dataTableName,formatted_init_datetime,formatted_end_datetime,data_aggregation_time_granularity,timeCriteria,cadena,sliceBy,neGroupingFilter,manageObjectId,dimFilter, elementType, vendor, equipmentId, locationId):
    query_druid=f'''{{
...
def data_load_control(param_df_threshold,var_datetime,aggregationTime,minutos):
    df = pd.read_json(param_df_threshold, orient='records')
    if len(df) != 0:
...
def druid_adapter_query(param_df_druid, aggregationTime, var_datetime, minutos):
    ...
    df = pd.read_json(param_df_druid, orient='records')
    if len(df) != 0:
        sql_queries = []       
        cadena = ''
        ...
        for _, row in df.iterrows():   
            ...
            percentil_time = ",\"percentile\":" + str(data_aggregation_percentile_value)
            ngInclusion = ""
            equimentInclusion = ""
            timeCriteria = "" 
            aggregation = ""
            ...
            print(f"typeAssociation: {typeAssociation}")
            ...
            cadena = f'''
                        {{  \"id\": \"{row['kpi_id']}\",
                            ...
                            \"temporalAggSpec\": {{
                                \"func\": \"{functionTime}\"
                                {percentil_time if data_aggregation_function != "NONE" and data_aggregation_function == "PERCENTILE" else ""}                                  
                            }},
                            \"spatialAggSpec\": {{
            ...
            query_druid = get_cadena_query_adapter(dataTableName, formatted_init_datetime, formatted_end_datetime, data_aggregation_time_granularity, timeCriteria, cadena, sliceBy, neGroupingFilter, manageObjectId, dimFilter, elementType, vendor, equipmentId, locationId)
            ...
            try:    
                ...
                if response.status_code == 200:
                    ...
                    query_update = f'''
                        UPDATE panda.control_load_data 
...
def execute_druid_query(sql_queries_str):    
    ...
    for sql in sql_queries:
        ...
        if len (lista_sql)>1:
            ...
            try:
                print("query SELECT 1:", lista_sql[0])     
                print("query SELECT 2:", lista_sql[1])     
...
def produce_message_to_topic_kafka(response_list, reportInterval, granularityInterval):       
    if len(response_list) > 0:
        ...
        for response in response_list:
            ...
            for index, row in df.iterrows(): 
                ...
                query = "SELECT load_status FROM panda.threshold WHERE node_id='" + node_id + "' AND kpi_id='" + row['metricId'] + "'"
                result = executeQuery(query)
                ...
                var_dn = json_data.get("dn", "default_dn")  
                var_threshold_id = json_data.get("thresholdId", "default_threshold")  

                key_str = f"{var_dn}+{var_threshold_id}"  
                key_bytes = key_str.encode('utf-8')  

                try:       
                    p.produce(topic, key=key_bytes, value=data_bytes, callback=delivery_report)                    
                    p.flush()   
                    status = "SUCESSFULL"
                    log = "Datos cargados con exito en Kafka al topico:" + topic        
                    query_update = f'''UPDATE panda.control_load_data SET status ='{status}', log ='{log}', pending = 0
                    {where}'''
                    print("status:", status)
                except Exception as e:
                    ...
                    query_update = f'''UPDATE panda.control_load_data SET status ='{status}', log ='{log}'
                    {where}''' 
                    raise e
                finally:
                    execute_Query(query_update,"","UPDATE",False)  
...
def insert_load_data(kpi_id,manageObject_id,var_data_time_granularity,networkgroups,equipments,v_delay,v_compleness,records_load_druid,status,formatted_init_datetime,formatted_end_datetime,aggregationTime,node_id,log,load_aggregationTime, load_status):
    data = f''' INSERT INTO panda.control_load_data (kpi_id, managedobject_id, granularityinterval, 
    ...
    query_update =f'''UPDATE PANDA.threshold 
...
def prepared_data_load(param_df_threshold,var_datetime,aggregationTime,minutos):
    def consultar_threshold(row):
        manageObject_Id = row['managedObject_id'] 
        data_aggregation_time_granularity = row['dataAggregationTimeGranularity'] if row['dataAggregationTimeGranularity']!="null" else row['kpi_defaultTimeGranularity']
        dataTableName = row['dataTableName']
        networkgroups = row['id_ngInclusion']
        equipments = row['equipmentInclusion']
    ...
    # Crear y ejecutar un hilo para cada objeto
    #df = pd.read_json(param_df_threshold, orient='records')
    df = pd.read_json(StringIO(param_df_threshold), orient='records')
    
    if len(df) != 0:
        hilos = [threading.Thread(target=consultar_threshold, args=(kpi,)) for index, kpi in df.iterrows()]
