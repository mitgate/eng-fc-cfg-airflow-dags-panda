...
import asyncio
import pytz
import functions_dev as functions_2
...
def insert_dataframe_to_postgres(**context):
    df = context['ti'].xcom_pull(task_ids='execute_graphql_query_task')
    df = pd.DataFrame(df)      
    df = df.fillna('null')

    delete_query = "DELETE FROM panda.threshold_dev;"
    functions_2.execute_Query(delete_query,"","DELETE", False)
    threshold_kpi_list=[]
    
    for index, row in df.iterrows():    
        if (row['node_id'] + "-" + row['kpi_name']) not in threshold_kpi_list:
            ...
            if row['dataAggregationTime'] == 'NONE' and row['spatialAggregation'] == 'NONE':
                ...
                data = f"""
                    INSERT INTO panda.threshold_dev (
                        node_id, 
                        node_name, 
                        "dataAggregationTime", 
                        "dataAggregationTimeGranularity", 
                    ...
                    VALUES (
                        '{row['node_id']}', 
                        '{row['node_name']}', 
                        '{row['dataAggregationTime']}', 
                        '{row['dataAggregationTimeGranularity']}',
                        '{row['dataAggregationFunction']}',
                         {row['dataAggregationPercentileValue']}, 
                        '{row['spatialAggregation']}',
                        '{row['spatialAggregationFunction']}', 
                         {row['spatialAggregationPercentileValue']}, 
                        '{row['id_spatialLocationTypeAggregation']}',
                        '{row['spatialLocationTypeAggregation']}', 
                        '{row['id_spatialManagedObjectAggregation']}', 
                        '{row['spatialManagedObjectAggregation']}',
                        '{row['managedObject_id']}',
                        '{row['managedObject_name']}', 
                        '{row['dataTableName']}', 
                        '{row['expression_id']}', 
                        '{row['ngExclusion']}', 
                        '{row['id_ngExclusion']}',
                        '{row['ngInclusion']}', 
                        '{row['id_ngInclusion']}', 
                        '{row['id_ngInclusion_SG']}', 
                        '{row['typeAssociation']}',
                        '{row['equipmentExclusion']}',
                        '{row['id_equipmentExclusion']}',
                        '{row['equipmentInclusion']}',
                        '{row['id_equipmentInclusion']}', 
                        '{row['kpi_id']}', 
                        '{row['kpi_name']}', 
                        '{row['kpi_defaultTimeGranularity']}', 
                        '{row['kpi_isRate']}',
                        '{row['kpi_unit']}',
                        '{row['kpi_hourly']}',
                        '{row['kpi_daily']}',
                        ...
                        '{row['kpi_quarterly']}',
                        '{row['kpi_semesterly']}',
                        '{row['hourlyPercentile']}',
                        '{row['dailyPercentile']}',
                        '{row['weeklyPercentile']}',
                        '{row['monthlyPercentile']}',
                        '{row['yearlyPercentile']}',
                        '{row['fifteenDaysPercentile']}',
                        '{row['quarterlyPercentile']}',
                        '{row['semesterlyPercentile']}',
                        '{row['algorithm']}',
                        '{row['algorithmComparisonValue']}',
                        '{row['algorithmComparisonType']}',
                        '{row['intervalDays']}',
                        '{row['times']}',
                        '{row['effectiveDays']}',
                        '{row['optionsDays']}',
                ...
                threshold_kpi_list.append(row['node_id']+"-"+row['kpi_name'])


#Definición del Dag
with DAG(
    dag_id = "dag_alarms_aggregation_dev_controller",
    default_args = default_args,
    start_date = datetime(2024, 4, 1),
    catchup = False,
    schedule_interval= None,  #'*/15 * * * *',  # Ejecutar cada 30 minuto
